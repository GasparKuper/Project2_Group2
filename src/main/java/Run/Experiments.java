    ...
    public void Solver() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int j = 0; j < 6; j++) {
                ...
                double tmpX = trajectoryOfAll[trajectoryOfAll.length - 1].celestialBody.get(3).position.getX() - earth1year[0];
                double tmpY = trajectoryOfAll[trajectoryOfAll.length - 1].celestialBody.get(3).position.getY() - earth1year[1];
                double tmpZ = trajectoryOfAll[trajectoryOfAll.length - 1].celestialBody.get(3).position.getZ() - earth1year[2];
                double errorX = Math.abs(tmpX)/Math.abs(earth1year[0]);
                double errorY = Math.abs(tmpY)/Math.abs(earth1year[1]);
                double errorZ = Math.abs(tmpZ)/Math.abs(earth1year[2]);
